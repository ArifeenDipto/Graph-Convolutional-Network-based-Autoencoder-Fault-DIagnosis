# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 20:38:41 2024
...
class gcnAE(torch.nn.Module):
    def __init__(self):
        ...
        self.enc3 = GCNConv(4, 2, improved=self.improved) #encoding layer 2
        self.bn3 = BatchNorm(2)
        
        self.mean = nn.Linear(2, 2) #mean sampling layer
        self.logstd = nn.Linear(2, 2)#standard dev sampling layer
        
        self.dec1 = GCNConv(2, 4, improved=self.improved) # deconding layer 1
    ...
    def forward(self, data, edg_idx, edg_wt):
        ...
        x5 = self.dec2(x4, edge_index, edge_weight)
        x5 = self.bn5(x5)
        x5 = F.tanh(x5)
                  
        x6 = self.dec3(x5, edge_index, edge_weight)
        return x6, z